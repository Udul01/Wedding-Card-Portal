	...
	<section id="content">
		...
		<main>
			<div class="container">
				...
				$sql = "SELECT id, bride_name, groom_name, invitees FROM wedding_cards"; // Selecting necessary fields including invitees
				...
				if ($result->num_rows > 0) {
				    ...
				    while ($row = $result->fetch_assoc()) {
				        ...
				        $invitees = explode(',', $row["invitees"]); // Split invitees into an array

				        // Generate link to individual wedding card page with groom's, bride's, and invitee's names and ID
				        foreach ($invitees as $invitee) {
				            $invitee = urlencode(trim($invitee)); // Encoding the invitee's name for URL
				            echo "<a class='wedding-card' href='wedding_card.php?id=$id&bride=$bride_name&groom=$groom_name&invitee=$invitee' target='_blank'>$bride_name & $groom_name's Wedding for $invitee</a><br>";
				        }
				    }
				} else {
